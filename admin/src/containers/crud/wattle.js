...
import { deleteBeak, getComb, updateBeak, addComb} from "../../actions/crud/wattleAttributes";
import Modal from "antd/lib/modal/Modal";

class WattleContainer extends Component {
  constructor(props) {
    super(props);
  ...
  async componentDidUpdate() {
    ...
    if (this.state.isDeleted || this.state.isNewAdded || this.state.isUpdated)
      if (this.props.isDeleted || this.props.isUpdated || this.props.isNewAdded) {
        ...
        //console.log('this is State', this.state.isUpdated)
      }
  }
  ...
  handleOk = () => {
    ...
    else if(this.state.modalTitle==="Add New Wattle"){
  ...
  render() {
    ...
    console.log(this.props.comb);
    return (
      <div className="right-section">
        <Alert type='error' message={this.props.alertMsg} showIcon={true} style={this.props.isError? { display: 'flex' }:{ display: 'none' }} />
        <Table dataSource={this.props.comb} columns={columns} /> <Button onClick={()=> this.addNewBeak()}>Add</Button>
        <Modal
...
function mapStateToProps(state) {
  ...
  return {
    comb: state.attribute.comb,
    isDeleted: state.attribute.beakDeleteStatus,
    isUpdated: state.attribute.wattleUpdateStatus,
    isNewAdded: state.attribute.newCombAdded,
    alertMsg: state.attribute.alertMsg, 
    isError: state.attribute.isError     
  };
}
export default connect(mapStateToProps)(WattleContainer);
